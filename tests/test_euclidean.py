...
ip1,ip2=input_value()
testdata1=[(ip1,ip1,0),(ip2,ip2,0),(ip1,ip2,2)]
testdata2=[(ip1,ip1,0),(ip2,ip2,0),(ip1,ip2,4)]

#check the euclidean distance
@pytest.mark.parametrize("sentence1, sentence2, output",testdata1)
def test_euclidean_distance_cls_token_embedding(sentence1,sentence2,output):
   score=cls_model.get_score(sentence1,sentence2,metric="euclidean")
   score=round(score,2)
   assert output-0.5 < score < output+0.5


@pytest.mark.parametrize("sentence1, sentence2, output",testdata2)
def test_euclidean_distance_sentence_embedding(sentence1,sentence2,output):
   score=sent_emb_model.get_score(sentence1,sentence2,metric="euclidean")
   score=round(score,2)
   assert output-0.5 < score < output+0.5
